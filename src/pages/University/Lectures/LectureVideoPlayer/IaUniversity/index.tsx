...
import {Span, Strong} from './Style';

const IaUniversity: SFC = ({className}) => {
  ...
  const [conversationId, setConversationId] = useState<number | null>(
    params.id ? parseInt(params.id, 10) : manager.activeConversationId || null,
  );
    ...
    try {
      ...
      if (!conversationId) {
        const conversation = await dispatch(
          createConversation({name: `Conversation Ia University ${conversationsLength + 1}`}),
        );
      ...
      <S.Bottom>
        ...
        >
          {({dirty, errors, isSubmitting, isValid, touched}) => (
            <S.Form>
              <S.TextInput errors={errors} name="text" placeholder="Please type here" touched={touched} />
              <S.Button
                dirty={dirty}
  ...
  const renderGreetingContainer = () => {
    return (
      <S.GreetingContainer>
        <S.GreetingElements>
          <S.Avatar>
            <S.Image alt="avatar" src={ia?.avatar || ''} height={35} width={35} />
          </S.Avatar>
          <S.GreetingText>
            Ia <Span>.</Span>
            <Strong>Now</Strong>
        ...
        <S.SubText>Hello I'm Ia! if you have questions about this video you may ask feel free!</S.SubText>
      </S.GreetingContainer>
    );
  };
